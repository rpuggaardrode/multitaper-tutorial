...
```{r}
mts_moments <- moments(mts_df$energy, mts_df$hz)
...
for (i in 1:steps) {
  ...
  tmp_mom <- moments(count=log(tmp_mts$spec), x=tmp_mts$freq)
  mom$step[i] <- i
  mom$cog[i] <- tmp_mom[1]
